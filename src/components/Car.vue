<template>
  <div class="view res-width lightbox">
    <p class="exitbtn mr-auto mb-3 pt-2" @click.self="closeLightbox">&#8249;&#8249; Back to Cars</p>
  ...
  computed: {
    photo() {
      return this.cars.find((car) => {
        return car.id === Number(this.$route.params.id);
      });
    },
  },
  methods: {
    ...
    prevLightbox() {
        ...
        if (this.photo.id < 14) {    //<---Number should be the final id
            this.$router.push('/cars/car/' + nextid)
        } 
    },
    nextLightbox() {
        var previd=this.photo.id-1
        if (this.photo.id > 0) {
            this.$router.push('/cars/car/' + previd)
        } 
    }
  },
};

<template>
  <div class="view res-width lightbox">
    <p class="exitbtn mr-auto mb-3 pt-2" @click.self="closeLightbox">&#8249;&#8249; Back to Watches</p>
  ...
  computed: {
    photo() {
      return this.watches.find((watch) => {
        return watch.id === Number(this.$route.params.id);
      });
    },
  },
  methods: {
    ...
    prevLightbox() {
        ...
        if (this.photo.id < 14) {    //<---Number should be the final id
            this.$router.push('/watches/watch/' + nextid)
        } 
    },
    nextLightbox() {
